...
function zrz_ajax_register(){
	...
	if($msg){
		...
		if (isset($user_id['errors'])) {
			...
			// 囤主题 www.tzhuti.com   自动登录
			wp_set_current_user($user_id);
            wp_set_auth_cookie($user_id);
            
            // 囤主题 www.tzhuti.com   更新邀请码
...
// 囤主题 www.tzhuti.com   有用户注册的时候，添加积分
...
function zrz_send_code(){
    ...
    if(is_email($email_or_phone)){
        ...
        // 囤主题 www.tzhuti.com    sprintf(__('<div style="margin-bottom:20px">[%1$s]：您的验证码为 <strong>%2$s</strong>，5分钟后失效，请尽快验证。</div>','ziranzhi2'),$site_name,$code);
...
// 囤主题 www.tzhuti.com   短信注册
function zrz_ali_sms($nub,$code){
    
    // 囤主题 www.tzhuti.com   生成验证码
    $mobile = $nub;

    if(!$code) return false;

    // 囤主题 www.tzhuti.com   选择
    $select = zrz_get_social_settings('sms_select');

    // 囤主题 www.tzhuti.com   发送短信
    if($select == 'aliyun'){
        ...
        $status = zrz_remote_post('https:// 囤主题 www.tzhuti.com   sms.yunpian.com/v2/sms/single_send.json',array('apikey'=>$sms['apikey'],'text'=>$msg,'mobile'=>$mobile,'register'=>true));
    }elseif($select == 'juhe'){
        $sms = zrz_get_social_settings('juhe');
        $status = zrz_remote_post('http:// 囤主题 www.tzhuti.com   v.juhe.cn/sms/send?',array('mobile'=>$mobile,'tpl_id'=>$sms['tpl_id'],'key'=>$sms['key'],'tpl_value'=>urlencode('#code#='.$code)));
    }
...
// 囤主题 www.tzhuti.com   使用邀请码
function zrz_sign_invitation($user_id,$inv){
    // 囤主题 www.tzhuti.com   检查邀请码是否使用
