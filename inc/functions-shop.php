...
// 囤主题 www.tzhuti.com   商品管理页面添加栏目属性
...
// 囤主题 www.tzhuti.com    // 囤主题 www.tzhuti.com   排序
// 囤主题 www.tzhuti.com    add_filter( 'manage_edit-shop_sortable_columns', 'zrz_set_shop_sortable_columns' );
// 囤主题 www.tzhuti.com    function zrz_set_shop_sortable_columns( $columns ) {
// 囤主题 www.tzhuti.com    	$columns['zrz_shop_type'] = 'zrz_shop_type';
// 囤主题 www.tzhuti.com    	$columns['zrz_shop_commodity'] = 'zrz_shop_commodity';
// 囤主题 www.tzhuti.com   
// 囤主题 www.tzhuti.com    	return $columns;
// 囤主题 www.tzhuti.com    }
// 囤主题 www.tzhuti.com   
// 囤主题 www.tzhuti.com    // 囤主题 www.tzhuti.com   排序事件
// 囤主题 www.tzhuti.com    add_action( 'pre_get_posts', 'zrz_shop_orderby' );
// 囤主题 www.tzhuti.com    function zrz_shop_orderby( $query ) {
// 囤主题 www.tzhuti.com    	if ( ! is_admin() )
// 囤主题 www.tzhuti.com    	return;
// 囤主题 www.tzhuti.com   
// 囤主题 www.tzhuti.com    	$orderby = $query->get( 'orderby');
// 囤主题 www.tzhuti.com   
// 囤主题 www.tzhuti.com    	if ($orderby == 'zrz_shop_type') {
// 囤主题 www.tzhuti.com    		$query->set( 'meta_key', 'zrz_shop_type');
// 囤主题 www.tzhuti.com    	}
// 囤主题 www.tzhuti.com    	$query->set( 'orderby', 'name' );
// 囤主题 www.tzhuti.com    }

// 囤主题 www.tzhuti.com    添加商品信息
...
function zrz_more_images_metabox($post){
	...
	$.fn.upload_listing_image = function( button ) {
		...
		// 囤主题 www.tzhuti.com    Create the media frame.
		...
		// 囤主题 www.tzhuti.com    When an image is selected, run a callback.
		...
		// 囤主题 www.tzhuti.com    Finally, open the modal
		file_frame.open();
	};
...
// 囤主题 www.tzhuti.com    保存填写的meta信息
...
function zrz_shop_metas_box_save($post_id){
	...
	// 囤主题 www.tzhuti.com   保存虚拟物品信息
	if(isset($_POST['zrz_shop_virtual_content']) && $_POST['zrz_shop_virtual_content']){
		$virtual = array(
			'title'=>isset($_POST['zrz_shop_virtual_title']) ? esc_attr($_POST['zrz_shop_virtual_title']) : '',// 囤主题 www.tzhuti.com   虚拟物品标题
			'type'=>isset($_POST['zrz_shop_virtual_type']) ? (int)$_POST['zrz_shop_virtual_type'] : '',// 囤主题 www.tzhuti.com   虚拟物品类型
			'content'=>isset($_POST['zrz_shop_virtual_content']) ? esc_attr($_POST['zrz_shop_virtual_content']) : ''// 囤主题 www.tzhuti.com   虚拟物品内容
		);

		update_post_meta($post_id,'zrz_shop_virtual',$virtual);
	}

	// 囤主题 www.tzhuti.com   商品数量
...
// 囤主题 www.tzhuti.com   获取商品价格
function zrz_get_shop_price($post_id,$type){
	$price = get_post_meta($post_id,'zrz_shop_price',true);
	if(!$price){
		$price = array(
			'price'=>'',// 囤主题 www.tzhuti.com   正常价格
			'd_price'=>'',// 囤主题 www.tzhuti.com   折后价格
			'u_price'=>'',// 囤主题 www.tzhuti.com   会员价格
			'credit'=>''// 囤主题 www.tzhuti.com   奖励的积分
		);
	}
...
// 囤主题 www.tzhuti.com   获取商品数量
function zrz_get_shop_count($post_id,$type){
	$count = get_post_meta($post_id,'zrz_shop_count',true);
	if(!$count){
		$count = array(
			'total'=>0,// 囤主题 www.tzhuti.com   总数量
			'sell'=>0,// 囤主题 www.tzhuti.com   已经出售或兑换的数量
			'sold_out'=>0
		);
	}
...
// 囤主题 www.tzhuti.com   获取抽奖数据
function zrz_get_shop_lottery($post_id,$type){
	$lottery = get_post_meta($post_id,'zrz_shop_lottery',true);
	if(!$lottery){
		$lottery = array(
			'probability'=>0.01,// 囤主题 www.tzhuti.com   中奖概率
			'credit'=>'',// 囤主题 www.tzhuti.com   所需积分
			'capabilities'=>array()
		);
	}
...
function zrz_get_shop_virtual($post_id,$type){
	...
	if(!$virtual){
		$virtual = array(
			'title'=>'',// 囤主题 www.tzhuti.com   虚拟物品标题
			'type'=>'',// 囤主题 www.tzhuti.com   虚拟物品类型
			'content'=>''// 囤主题 www.tzhuti.com   虚拟物品内容
		);
	}
