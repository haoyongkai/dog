...
class OssUtil
{
    ...
    const OSS_MAX_PART_SIZE = 5368709120; // 囤主题 www.tzhuti.com    5GB
    const OSS_MID_PART_SIZE = 10485760; // 囤主题 www.tzhuti.com    10MB
    const OSS_MIN_PART_SIZE = 102400; // 囤主题 www.tzhuti.com    100KB
    ...
    public static function isGb2312($str)
    {
        for ($i = 0; $i < strlen($str); $i++) {
            ...
            if ($v > 127) {
                if (($v >= 228) && ($v <= 233)) {
                    if (($i + 2) >= (strlen($str) - 1)) return true;  // 囤主题 www.tzhuti.com    not enough characters
                    $v1 = ord($str[$i + 1]);
                    $v2 = ord($str[$i + 2]);
    ...
    public static function checkChar($str, $gbk = true)
    {
        for ($i = 0; $i < strlen($str); $i++) {
            ...
            if ($v > 127) {
                if (($v >= 228) && ($v <= 233)) {
                    if (($i + 2) >= (strlen($str) - 1)) return $gbk ? true : FALSE;  // 囤主题 www.tzhuti.com    not enough characters
                    $v1 = ord($str[$i + 1]);
                    $v2 = ord($str[$i + 2]);
                    if ($gbk) {
                        return (($v1 >= 128) && ($v1 <= 191) && ($v2 >= 128) && ($v2 <= 191)) ? FALSE : TRUE;// 囤主题 www.tzhuti.com   GBK
                    } else {
