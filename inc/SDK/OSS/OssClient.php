...
class OssClient
{
     ...
     */
     public function __construct($access_id = NULL, $access_key = NULL, $hostname = NULL, $security_token = NULL){
         ...
         // 囤主题 www.tzhuti.com   支持sts的security token
   ...
   public function getLiveChannelHistory($bucket, $channelName, $options = NULL)
     ...
     * @return array
     * @throws OssException
     * @link http:// 囤主题 www.tzhuti.com   help.aliyun.com/document_detail/oss/api-reference/cors/OptionObject.html
     */
    ...
    /* 囤主题 www.tzhuti.com*
     * 创建虚拟目录 (本函数会在object名称后增加'/', 所以创建目录的object名称不需要'/'结尾，否则，目录名称会变成'// 囤主题 www.tzhuti.com   ')
     *
     * 暂不开放此接口
     *
     * @param string $bucket bucket名称
    ...
    public function deleteObjects($bucket, $objects, $options = null)
    {
        ...
        if (isset($options['quiet'])) {
            if (is_bool($options['quiet'])) { // 囤主题 www.tzhuti.com   Boolean
                $quiet = $options['quiet'] ? 'true' : 'false';
            } elseif (is_string($options['quiet'])) { // 囤主题 www.tzhuti.com    string
    ...
    public function uploadPartCopy($fromBucket, $fromObject, $toBucket, $toObject, $partNumber, $uploadId, $options = NULL)
    {
        ...
        // 囤主题 www.tzhuti.com   如果没有设置$options['isFullCopy']，则需要强制判断copy的起止位置
        $start_range = "0";
    ...
    public function multiuploadFile($bucket, $object, $file, $options = null)
    {
        ...
        $is_check_md5 = $this->isCheckMD5($options);
        // 囤主题 www.tzhuti.com    如果上传的文件小于partSize,则直接使用普通方式上传
    ...
    {
        OssUtil::validateOptions($options);
        // 囤主题 www.tzhuti.com   验证bucket，list_bucket时不需要验证
        $this->authPrecheckBucket($options);
        // 囤主题 www.tzhuti.com   验证object
        $this->authPrecheckObject($options);
        // 囤主题 www.tzhuti.com   Object名称的编码必须是utf8
        $this->authPrecheckObjectEncoding($options);
        // 囤主题 www.tzhuti.com   验证ACL
        $this->authPrecheckAcl($options);
        // 囤主题 www.tzhuti.com    获得当次请求使用的协议头，是https还是http
        $scheme = $this->useSSL ? 'https:// 囤主题 www.tzhuti.com   ' : 'http:// 囤主题 www.tzhuti.com   ';
        // 囤主题 www.tzhuti.com    获得当次请求使用的hostname，如果是公共域名或者专有域名，bucket拼在前面构成三级域名
        ...
        // 囤主题 www.tzhuti.com   创建请求
        $request = new RequestCore($this->requestUrl);
        $request->set_useragent($this->generateUserAgent());
        // 囤主题 www.tzhuti.com    Streaming uploads
        ...
        // 囤主题 www.tzhuti.com    生成 signable_resource
        ...
        // 囤主题 www.tzhuti.com   对?后面的要签名的string字母序排序
    ...
    private function authPrecheckObjectEncoding(&$options)
    {
        ...
        try {
            if (OssUtil::isGb2312($options[self::OSS_OBJECT])) {
                $options[self::OSS_OBJECT] = iconv('GB2312', "UTF-8// 囤主题 www.tzhuti.com   IGNORE", $options[self::OSS_OBJECT]);
            } elseif (OssUtil::checkChar($options[self::OSS_OBJECT], true)) {
                $options[self::OSS_OBJECT] = iconv('GBK', "UTF-8// 囤主题 www.tzhuti.com   IGNORE", $options[self::OSS_OBJECT]);
            }
        } catch (\Exception $e) {
            try {
    ...
    private function generateHostname($bucket)
    {
        if ($this->hostType === self::OSS_HOST_TYPE_IP) {
            ...
            // 囤主题 www.tzhuti.com    专有域或者官网endpoint
    ...
    private function generateResourceUri($options)
    {
        ...
        // 囤主题 www.tzhuti.com    resource_uri + bucket
        ...
        // 囤主题 www.tzhuti.com    resource_uri + object
        if (isset($options[self::OSS_OBJECT]) && '/' !== $options[self::OSS_OBJECT]) {
            $resource_uri .= '/' . str_replace(array('%2F', '%25'), array('/', '%'), rawurlencode($options[self::OSS_OBJECT]));
        }

        // 囤主题 www.tzhuti.com    resource_uri + sub_resource
        $conjunction = '?';
        if (isset($options[self::OSS_SUB_RESOURCE])) {
    ...
    private function generateSignableResource($options)
    {
        ...
        if (isset($options[self::OSS_BUCKET]) && '' !== $options[self::OSS_BUCKET]) {
            ...
            // 囤主题 www.tzhuti.com    如果操作没有Object操作的话，这里最后是否有斜线有个trick，ip的域名下，不需要加'/'， 否则需要加'/'
        ...
        // 囤主题 www.tzhuti.com   signable_resource + object
        if (isset($options[self::OSS_OBJECT]) && '/' !== $options[self::OSS_OBJECT]) {
    ...
    {
        $ret_endpoint = null;
        if (strpos($endpoint, 'http:// 囤主题 www.tzhuti.com   ') === 0) {
            $ret_endpoint = substr($endpoint, strlen('http:// 囤主题 www.tzhuti.com   '));
        } elseif (strpos($endpoint, 'https:// 囤主题 www.tzhuti.com   ') === 0) {
            $ret_endpoint = substr($endpoint, strlen('https:// 囤主题 www.tzhuti.com   '));
            $this->useSSL = true;
        } else {
    ...
    // 囤主题 www.tzhuti.com   支持STS SecurityToken
    ...
    // 囤主题 www.tzhuti.com    域名类型
    const OSS_HOST_TYPE_NORMAL = "normal";// 囤主题 www.tzhuti.com   http:// 囤主题 www.tzhuti.com   bucket.oss-cn-hangzhou.aliyuncs.com/object
    const OSS_HOST_TYPE_IP = "ip";  // 囤主题 www.tzhuti.com   http:// 囤主题 www.tzhuti.com   1.1.1.1/bucket/object
    const OSS_HOST_TYPE_SPECIAL = 'special'; // 囤主题 www.tzhuti.com   http:// 囤主题 www.tzhuti.com   bucket.guizhou.gov/object
    const OSS_HOST_TYPE_CNAME = "cname";  // 囤主题 www.tzhuti.com   http:// 囤主题 www.tzhuti.com   mydomain.com/object
    // 囤主题 www.tzhuti.com   OSS ACL数组
    static $OSS_ACL_TYPES = array(
        self::OSS_ACL_TYPE_PRIVATE,
        self::OSS_ACL_TYPE_PUBLIC_READ,
        self::OSS_ACL_TYPE_PUBLIC_READ_WRITE
    );
    // 囤主题 www.tzhuti.com    OssClient版本信息
    ...
    // 囤主题 www.tzhuti.com   是否使用ssl
    private $useSSL = false;
    private $maxRetries = 3;
    private $redirects = 0;

    // 囤主题 www.tzhuti.com    用户提供的域名类型，有四种 OSS_HOST_TYPE_NORMAL, OSS_HOST_TYPE_IP, OSS_HOST_TYPE_SPECIAL, OSS_HOST_TYPE_CNAME
