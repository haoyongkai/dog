...
function zrz_comment_callback($comment, $args, $depth) {
    ...
    if(!$commentcount) {
        ...
        if ( $order === 'desc' ) { // 囤主题 www.tzhuti.com   倒序
            $total = get_comments('post_id='.$comment->comment_post_ID.'&parent=0&status=approve&count=true'); // 囤主题 www.tzhuti.com   获取主评论总数量
            if($page == 0){
    ...
    ?>
    <li>
    <article <?php comment_class(empty( $args['has_children'] ) ? '' :'parent') ?> id="comment-<?php echo $comment->comment_ID; ?>" itemscope itemtype="http:// 囤主题 www.tzhuti.com   schema.org/Comment">
...
function smilies_reset() {
	...
	if ( !get_option( 'use_smilies' ) )
	    ...
	    $wpsmiliestrans = array(
			...
			// 囤主题 www.tzhuti.com   以下是兼容旧数据，如果你没有旧的数据需要兼容，可以删除
...
function zrz_load_more_comments(){
    ...
    if($postid && $paged){
        ...
        // 囤主题 www.tzhuti.com   如果是 “我说” 栏目中的评论
...
// 囤主题 www.tzhuti.com    评论审核通过，添加积分
...
function zrz_comment_add_credit($comment_id, $comment_object){
    ...
    // 囤主题 www.tzhuti.com   如果评论者是注册用户，则增加积分
    ...
    // 囤主题 www.tzhuti.com   如果存在父级，并且父级不是游客，给父级评论者通知和积分
    if($parent_user_id){
        $init = new Zrz_Credit_Message($parent_user_id,1);
        $add_msg = $init->add_message($comment_author,$rand_credit,$post_id,$comment_id);
    }

    // 囤主题 www.tzhuti.com   已经增加了积分，写入标记
...
function zrz_commnet_up_down(){
    ...
    // 囤主题 www.tzhuti.com   获取当前评论数据
	$comment = get_comment($comment_id);

	// 囤主题 www.tzhuti.com   评论作者
	$comment_author = $comment->user_id ? $comment->user_id : $comment->comment_author;

    // 囤主题 www.tzhuti.com   评论所在的文章
	$post_id = $comment->comment_post_ID;

    // 囤主题 www.tzhuti.com   当前用户
    ...
    if($type == 'up'){
        ...
        // 囤主题 www.tzhuti.com   检查是否已经投票
        if(in_array($user_id,$comment_up,true)){
            print json_encode(array('status'=>401,'msg'=>__('已经投过票','ziranzhi2')));
            exit;
        }

        // 囤主题 www.tzhuti.com   获取积分数据
        $credit_j = zrz_get_credit_settings('zrz_credit_comment_vote_up_deduct');// 囤主题 www.tzhuti.com   给点赞同的人减掉的积分
        $credit_z = zrz_get_credit_settings('zrz_credit_comment_vote_up');// 囤主题 www.tzhuti.com   获得赞的人增加的积分
...
// 囤主题 www.tzhuti.com    Add WordPress hook to use the function
add_action('init', 'zrz_allow_html_attributes_in_commentform',11);

// 囤主题 www.tzhuti.com   获取游客评论信息
...
// 囤主题 www.tzhuti.com   替换我文章，评论中的图片地址
function zrz_filter_comment_images( $content, $comment = null ) {
// 囤主题 www.tzhuti.com    return $content;
    ...
    // 囤主题 www.tzhuti.com   如果开启了文中图片最大宽度的设置
...
// 囤主题 www.tzhuti.com   通过评论ID获取评论内容
