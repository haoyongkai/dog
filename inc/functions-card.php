<?php
// 囤主题 www.tzhuti.com   生成卡密
function zrz_create_guid($namespace = null,$inv = false) {
    static $guid = '';
    $uid = uniqid ( "", true );

    $data = $namespace;
    $data .= $_SERVER ['REQUEST_TIME'];     // 囤主题 www.tzhuti.com    请求那一刻的时间戳
    $data .= $_SERVER ['HTTP_USER_AGENT'];  // 囤主题 www.tzhuti.com    获取访问者在用什么操作系统
    $data .= $_SERVER ['SERVER_ADDR'];      // 囤主题 www.tzhuti.com    服务器IP
    $data .= $_SERVER ['SERVER_PORT'];      // 囤主题 www.tzhuti.com    端口号
    $data .= $_SERVER ['REMOTE_ADDR'];      // 囤主题 www.tzhuti.com    远程IP
    $data .= $_SERVER ['REMOTE_PORT'];      // 囤主题 www.tzhuti.com    端口信息
