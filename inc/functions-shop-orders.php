...
/* 囤主题 www.tzhuti.com
* 商城订单项
* $order_type // 囤主题 www.tzhuti.com   订单类型
* c : 抽奖 ，d : 兑换 ，g : 购买 ，w : 文章内购 ，ds : 打赏 ，cz : 充值 ，vip : VIP购买 ,cg : 积分购买
*
* $order_commodity // 囤主题 www.tzhuti.com   商品类型
* 0 : 虚拟物品 ，1 : 实物
*
* $order_state // 囤主题 www.tzhuti.com   订单状态
...
class Zrz_order_Message{
    ...
    private $user_id;// 囤主题 www.tzhuti.com   用户ID
    private $post_id;// 囤主题 www.tzhuti.com   用户ID
    private $type;// 囤主题 www.tzhuti.com   订单类型
    private $order_commodity;// 囤主题 www.tzhuti.com   商品类型
    private $date;// 囤主题 www.tzhuti.com   时间
    private $order_id;// 囤主题 www.tzhuti.com   订单号
    private $order_count;// 囤主题 www.tzhuti.com   订单数量
    private $order_price;// 囤主题 www.tzhuti.com   订单价格
    private $order_state;// 囤主题 www.tzhuti.com   订单状态
    private $key;// 囤主题 www.tzhuti.com   自定义的 KEY
    private $value;// 囤主题 www.tzhuti.com   订自定义的 value
    private $order_content;// 囤主题 www.tzhuti.com   买家留言

    private $wpdb;// 囤主题 www.tzhuti.com   数据库全局变量
    private $table_name;// 囤主题 www.tzhuti.com   表名
...
class Zrz_Order_List_Table extends WP_List_Table {
    ...
    // 囤主题 www.tzhuti.com   列表数据
    function prepare_items($val = '') {
        // 囤主题 www.tzhuti.com   每页显示多少条
        $per_page = 10;
        ...
        // 囤主题 www.tzhuti.com   当前页面
        $current_page = $this->get_pagenum();

        // 囤主题 www.tzhuti.com   总数
...
// 囤主题 www.tzhuti.com   记录已经购买或者兑换或者抽奖的用户
...
// 囤主题 www.tzhuti.com   商品支付成功，回调函数。 $pay_order_id : 支付订单号，$total_amount : 支付的金额
function zrz_notify_data_update($pay_order_id,$total_amount){
    ...
    // 囤主题 www.tzhuti.com   购买会员回调
		...
		if($resout){
			...
			// 囤主题 www.tzhuti.com   记录购买时间和到期时间
    ...
    if(isset($order_data_total['order']) && $order_data_total['order'] == $pay_order_id){
        ...
        if($order_data['type'] == 'post'){
            ...
            // 囤主题 www.tzhuti.com   如果是商品购买
            // 囤主题 www.tzhuti.com   获取邮寄地址
            $add = zrz_get_default_address($user_id);
            $i = 0;
            foreach ($ids as $val) {
                $i++;
...
// 囤主题 www.tzhuti.com   使用帐号余额支付付费文章
...
function zrz_post_pay_with_balance(){
    ...
    // 囤主题 www.tzhuti.com   检查文章所需的金额
    $cap = get_post_meta($post_id,'capabilities',true);

    // 囤主题 www.tzhuti.com   检查文章类型
    if(isset($cap['key']) && isset($cap['val']) && $cap['key'] == 'rmb'){
        $toatl_price = $cap['val'];
        // 囤主题 www.tzhuti.com   支付订单号
        $id = array(
            'id'=>$post_id,
            'count'=>1
        );
    ...
    // 囤主题 www.tzhuti.com   删除订单信息
    delete_user_meta($user_id,'zrz_orders');

    // 囤主题 www.tzhuti.com   删除订单状态
    delete_user_meta($user_id,'zrz_ds_resout');

    $user_order = array(
        'order'=>$order_id,
        'ids'=>$id_arr,// 囤主题 www.tzhuti.com   商品ID
        'total_price'=>$toatl_price,// 囤主题 www.tzhuti.com   支付金额
        'balance'=>0,// 囤主题 www.tzhuti.com   是否使用余额
        'payed'=>0,
        'type'=>'post'
    );

    // 囤主题 www.tzhuti.com   设置一个临时数据，回调的时候检查
    update_user_meta($user_id,'zrz_orders',$user_order);

    // 囤主题 www.tzhuti.com   先付费阅读信息打赏信息标记
    update_user_meta($user_id,'zrz_ds_resout','begin');

    $c_order_id = zrz_build_order_no();

    // 囤主题 www.tzhuti.com   生成一个临时的订单
...
function zrz_ds_with_balance(){
    ...
    // 囤主题 www.tzhuti.com   先清除打赏信息
    delete_user_meta($user_id,'zrz_ds_resout');

    // 囤主题 www.tzhuti.com   记录留言信息
...
function zrz_buy_vip_by_balance(){
    ...
    // 囤主题 www.tzhuti.com   检查余额是否充足
    $rmb = get_user_meta($user_id,'zrz_rmb',true);
    $rmb = $rmb ? $rmb : 0;

    // 囤主题 www.tzhuti.com   如果余额不足，结束支付
    if($rmb < $price){
        print json_encode(array('status'=>401,'msg'=>__('余额不足','ziranzhi2')));
        exit;
    }
...
function zrz_tx_application(){
    ...
    // 囤主题 www.tzhuti.com   检查是否有未完成的提现
