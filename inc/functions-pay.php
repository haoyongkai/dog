...
function zrz_back_qcode($order_id,$toatl_price,$title,$current_url,$paytype){
    ...
    if($alipay == 1 && $current_url == 'http:// 囤主题 www.tzhuti.com   alipayscan'){
        ...
        $options = array(
            'out_trade_no' => $order_id, // 囤主题 www.tzhuti.com    商户订单号
            'total_amount' => $toatl_price*0.01, // 囤主题 www.tzhuti.com    支付金额
            'subject'      => $title, // 囤主题 www.tzhuti.com    支付订单描述
            'notify_url'   => home_url('/notify-pay'), // 囤主题 www.tzhuti.com    定义通知URL
        );
    ...
    if($type == 'weixin'){
        ...
        if(wp_is_mobile()){
            ...
            // 囤主题 www.tzhuti.com   公众号支付
            if(zrz_is_weixin()){
                $cuser_id = get_current_user_id();
                // 囤主题 www.tzhuti.com    支付参数
                $options = [
                    'out_trade_no'     => $order_id, // 囤主题 www.tzhuti.com    订单号
                    'total_fee'        => $toatl_price, // 囤主题 www.tzhuti.com    订单金额，**单位：分**
                    'body'             => $title, // 囤主题 www.tzhuti.com    订单描述
                    'spbill_create_ip' => $ip, // 囤主题 www.tzhuti.com    支付人的 IP
                    'openid'           => get_user_meta($cuser_id,'zrz_weixin_open_id',true), // 囤主题 www.tzhuti.com    支付人的 openID
                    'notify_url'       => home_url('/weixinpay-notify'), // 囤主题 www.tzhuti.com    定义通知URL
                ];
    ...
    // 囤主题 www.tzhuti.com   payjs 支付
    if($type == 'payjs'){
        require ZRZ_THEME_DIR.'/inc/SDK/Payjs/payjs.php';
        $arr =array(
           'body' =>$title,               // 囤主题 www.tzhuti.com    订单标题
           'out_trade_no' =>$order_id,       // 囤主题 www.tzhuti.com    订单号
           'total_fee' => $toatl_price,// 囤主题 www.tzhuti.com    金额,单位:分
    ...
    if($type == 'youzan'){
        ...
        if (!isset($resp['response'])) {
            ...
            // 囤主题 www.tzhuti.com   保存 qr_id 到临时订单
...
function zrz_weixin_post_pay(){
    ...
    // 囤主题 www.tzhuti.com   删除旧订单信息
    delete_user_meta($user_id,'zrz_orders');

    // 囤主题 www.tzhuti.com   开始订单状态
    delete_user_meta($user_id,'zrz_ds_resout');

    $user_order = array(
        'order'=>$order_id,
        'ids'=>$id_arr,// 囤主题 www.tzhuti.com   商品ID
        'total_price'=>$toatl_price/100,// 囤主题 www.tzhuti.com   支付金额
        'balance'=>$balance,// 囤主题 www.tzhuti.com   是否使用余额
        'payed'=>0,
        'type'=>$pay_type
    );
