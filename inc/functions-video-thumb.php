<?php
// 囤主题 www.tzhuti.com   视频网站截图获取
// 囤主题 www.tzhuti.com   版本：V1.0
// 囤主题 www.tzhuti.com   开发者：顾问
// 囤主题 www.tzhuti.com   官方网站：https:// 囤主题 www.tzhuti.com   www.dounai0.com
// 囤主题 www.tzhuti.com   502网站无法访问
...
function zrz_get_video_thumb($_url){
    ...
    // 囤主题 www.tzhuti.com   优酷
    if($url['host']=="v.youku.com"){
    	$youkuid = g($url['path'],'|/v_show/id_(.*).html|i',1);
    	$youkupic = json_decode(zrz_get_url_content("https:// 囤主题 www.tzhuti.com   api.youku.com/videos/show.json?client_id=58263aed0903a6d8&video_id=$youkuid"),true);
        $title = $youkupic['title'];
    	$bigThumbnail = $youkupic['bigThumbnail'];
    }
    // 囤主题 www.tzhuti.com   土豆
    elseif($url['host']=="new-play.tudou.com" || $url['host']=="video.tudou.com"){
    	$tudouid = g($url['path'],'|/v/(.*).html|i',1);
    	$tudoupic = json_decode(zrz_get_url_content("https:// 囤主题 www.tzhuti.com   api.youku.com/videos/show.json?client_id=58263aed0903a6d8&video_id=$tudouid"),true);
        $title = $youkupic['title'];
        $bigThumbnail = $tudoupic['bigThumbnail'];
    }
    // 囤主题 www.tzhuti.com   qq
    elseif($url['host']=="v.qq.com"){
    	$qqidurl = zrz_get_url_content($_url);
    	preg_match_all("|\"pic_640_360\"\:\"(.*)\",\"c_title_segment|U", $qqidurl, $regs2);// 囤主题 www.tzhuti.com   获取网站大图
        preg_match("/<title>(.+)<\/title>/i", $qqidurl, $matches);
    	$bigThumbnail = $regs2[1][0];
        $title = $matches[1];
    }
    // 囤主题 www.tzhuti.com   奇艺
    elseif($url['host']=="www.iqiyi.com"){
    	$qiyiidurl = zrz_get_url_content($_url);
    	preg_match_all("|<meta itemprop=\"image\" content=\"(.*)\"\/>|U", $qiyiidurl, $regs2);// 囤主题 www.tzhuti.com   获取网站大图
