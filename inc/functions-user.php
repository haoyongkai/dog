...
// 囤主题 www.tzhuti.com   将用户页面的 author 改为 user
function change_author_permalinks() {
    global $wp_rewrite;
    $wp_rewrite->author_base = 'user';
    // 囤主题 www.tzhuti.com   $wp_rewrite->flush_rules();
}
add_action('init','change_author_permalinks');

add_filter('query_vars', 'users_query_vars');
function users_query_vars($vars) {
    // 囤主题 www.tzhuti.com    add lid to the valid list of variables
    $vars[] = 'user';
    $vars[] = 'zrz_user_page';
    return $vars;
}
...
// 囤主题 www.tzhuti.com   更新用户信息
class zrz_update_user_data{
    public static $type;
    public static $val;// 囤主题 www.tzhuti.com   存入的值
    public static $user_id;// 囤主题 www.tzhuti.com   用户ID
    public static $avatar_type;// 囤主题 www.tzhuti.com   头像类型
    public static $avatar_new;// 囤主题 www.tzhuti.com   社交头像原始图片链接，用以对比用户的社交网络上的头像是否更换
    ...
    public static function update_user_meta(){
        ...
        switch (self::$type) {
            // 囤主题 www.tzhuti.com   更新用户封面图片 cover
            case 'cover':
                ...
                break;
            // 囤主题 www.tzhuti.com   更新用户头像 avatar
            case 'avatar':
...
class zrz_get_user_data{
    public static $user_id;// 囤主题 www.tzhuti.com   用户ID
    public static $user_open;// 囤主题 www.tzhuti.com   用户的自定义信息，包括封面图片，头像，社交登陆的信息等等
    public static $size;// 囤主题 www.tzhuti.com   头像尺寸
    ...
    // 囤主题 www.tzhuti.com   获取用户封面图片
    public static function get_cover(){

        // 囤主题 www.tzhuti.com   如果没有封面图片，返回空
...
function zrz_ajax_update_user_data(){
    ...
    // 囤主题 www.tzhuti.com   开启了何种验证方式
    $sign_type = zrz_get_social_settings('type');
    $pass = '';

    // 囤主题 www.tzhuti.com   修改邮箱或者电话
    if (!isset($_SESSION)) {
        session_start();
    }
...
function zrz_setting_change_nub(){
    ...
    // 囤主题 www.tzhuti.com   积分变更
    if($type == 'credit'){
        // 囤主题 www.tzhuti.com   添加通知
...
// 囤主题 www.tzhuti.com   关注
add_action('wp_ajax_zrz_follow','zrz_follow');
function zrz_follow(){
    // 囤主题 www.tzhuti.com   被关注者的ID
    $user_id = isset($_POST['user_id']) ? (int)$_POST['user_id'] : 0;
    ...
    // 囤主题 www.tzhuti.com   如果未关注，则添加关注
        $follow[] = $user_id;
        $fans[] = $current_id;
        $type = 'add';
    }
    ...
    if($type == 'add'){
        ...
        // 囤主题 www.tzhuti.com   减去自己的积分
        $init = new Zrz_Credit_Message($current_id,43);
		$add_msg = $init->add_message($user_id, -$c_credit,0,0);
    }

    // 囤主题 www.tzhuti.com   更新数据
...
// 囤主题 www.tzhuti.com   允许搜索display_name
...
function zrz_send_msg(){
    ...
    // 囤主题 www.tzhuti.com   检查是不是垃圾
    $user_custom_data = get_user_meta($current_user_id,'zrz_user_custom_data',true);
    $url = preg_replace( '/^https?:\/\// 囤主题 www.tzhuti.com   ', '', home_url() );
