...
// 囤主题 www.tzhuti.com   内页上一篇文章，下一批文章
function zrz_get_pre_next_post(){
	...
	// 囤主题 www.tzhuti.com   如果没有上一篇或者下一篇，则显示随机文章
	if(!$pre_id){
...
function zrz_get_post_des($post_id){
	...
	// 囤主题 www.tzhuti.com   如果存在SEO描述输出，否则输出文章摘要，否则输出文章内容截断
...
function zrz_get_share($type = false){
	...
	if($type){
		return 'http:// 囤主题 www.tzhuti.com   service.weibo.com/share/share.php?url='.$url.'&coun=1&pic='.zrz_get_post_thumb_img($post->ID).'&title='.$post_title;
	}else{
		$weibo = 'http:// 囤主题 www.tzhuti.com   service.weibo.com/share/share.php?url='.$url.'&coun=1&pic='.$thumb.'&title='.$post_title;
	}

    $qqzone = 'http:// 囤主题 www.tzhuti.com   sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url='.$url.'&desc=来<'.$site_name.'>看看这篇文章吧，有惊喜哦！&site='.$site_name.'&summary='.$post_content.'&pics='.$thumb.'&title='.$post_title;

    $qq = 'http:// 囤主题 www.tzhuti.com   connect.qq.com/widget/shareqq/index.html?url='.$url.'&desc=来<'.$site_name.'>看看这篇文章吧，有惊喜哦！&title='.$post_title.'&summary='.$post_content.'&pics='.$thumb.'&site='.$site_name;
...
function zrz_add_post_face(){
	...
	if( is_user_logged_in() ) {
		...
		if(isset($user_faces[$post_id])){
			...
			// 囤主题 www.tzhuti.com   如果 user_meta 中包含当前元素，则删除当前元素
			$ac = false;
			if(in_array($face,$user_arr,true)){
				// 囤主题 www.tzhuti.com   键值互换
...
function zrz_post_favorites(){
	...
	if(!in_array($user_id, $post_favorites,true)){// 囤主题 www.tzhuti.com   检查当前用户是否已经点赞，如果没有点赞则执行加入
	...
	// 囤主题 www.tzhuti.com   更新文章收藏数据
	update_post_meta($post_id, 'zrz_favorites', $post_favorites);

	// 囤主题 www.tzhuti.com   更新当前用户收藏的文章数据
...
// 囤主题 www.tzhuti.com   通过ID获取文章或商品
...
function zrz_publish_post( $new_status, $old_status, $post ){
	...
	// 囤主题 www.tzhuti.com   过滤掉机器和其他地方发表的文章
	...
	// 囤主题 www.tzhuti.com   如果增加过积分
    if(get_post_meta($post->ID,'zrz_credit_add',true)) return;

    // 囤主题 www.tzhuti.com   文章作者
    $user_id = $post->post_author;
    $post_type = $post->post_type;

    switch ($post_type) {
        case 'post':
            // 囤主题 www.tzhuti.com   发表文章时要添加的积分
            $credit = zrz_get_credit_settings('zrz_credit_post');
            // 囤主题 www.tzhuti.com   文章分类
            $id = $post->ID;
            $type = 5;
            break;
        case 'topic':
            // 囤主题 www.tzhuti.com   发表帖子时要添加的积分
            $credit = zrz_get_credit_settings('zrz_credit_topic');
            $id = $post->ID;
            $type = 17;
            break;
        case 'reply':
            // 囤主题 www.tzhuti.com   发表帖子回复时要添加的积分
            ...
            // 囤主题 www.tzhuti.com   回帖者给自己回复不添加积分，给话题作者积分和通知
            ...
            // 囤主题 www.tzhuti.com   提到了某人，给某人加积分和通知
            ...
            break;
        case 'labs':
            // 囤主题 www.tzhuti.com   发表研究时要添加的积分
            $credit = zrz_get_credit_settings('zrz_credit_labs');
            // 囤主题 www.tzhuti.com   文章id
            $id = $post->ID;
            $type = 36;
            break;

		case 'pps':
			// 囤主题 www.tzhuti.com   发表冒泡时要添加的积分
...
function zrz_admin_post_init() {
    ...
    if(in_array($pagenow, array('post-new.php','edit.php','post.php'))){
		// 囤主题 www.tzhuti.com   后台发布的文章，做一个标记
		if (!isset($_SESSION)) {
		    session_start();
		}
...
// 囤主题 www.tzhuti.com   检查编辑时间是否超时，返回 负数 则代表超时，返回 正数 则代表未超时
...
function zrz_insert_post(){
	...
	// 囤主题 www.tzhuti.com    if(!current_user_can('edit_users')){
	// 囤主题 www.tzhuti.com    	// 囤主题 www.tzhuti.com   检查是否有草稿
	// 囤主题 www.tzhuti.com    	$args=array(
	// 囤主题 www.tzhuti.com    		'post_type' => 'post',
	// 囤主题 www.tzhuti.com    		'post_status' => 'pending',
	// 囤主题 www.tzhuti.com    		'posts_per_page' => 5,
	// 囤主题 www.tzhuti.com    		'author' => $user_id
	// 囤主题 www.tzhuti.com    	);

	// 囤主题 www.tzhuti.com    	$posts = get_posts($args);
	// 囤主题 www.tzhuti.com    	if(count($posts) > 3){
	// 囤主题 www.tzhuti.com    		print json_encode(array('status'=>401,'msg'=>__('有待审文章还未审核，不再次提交','ziranzhi2')));
	// 囤主题 www.tzhuti.com    		exit;
	// 囤主题 www.tzhuti.com    	}
	// 囤主题 www.tzhuti.com    }
	
	// 囤主题 www.tzhuti.com   检查是不是机器操作
	check_ajax_referer($user_id, 'security' );
	...
	// 囤主题 www.tzhuti.com   编辑模式
	if($post_id){

		// 囤主题 www.tzhuti.com   检查文章作者
		...
		// 囤主题 www.tzhuti.com   时间，如果是文章修改，则不改变时间，否则时间为当前
	...
	// 囤主题 www.tzhuti.com   前台发布的文章，做一个标记
	if (!isset($_SESSION)) {
	    session_start();
	}
	$_SESSION['zrz_credit_add'] = 1;
...
if(!function_exists('zrz_content_hide')){
	function zrz_content_hide( $atts, $content = null ) {
		...
		* 积分支付可见
		*/
		// 囤主题 www.tzhuti.com   如果是积分支付
		if($cap['key'] == 'credit'){
			$credit = $cap['val'];

			// 囤主题 www.tzhuti.com   检查积分剩余
			$user_credit = (int)zrz_coin($user_id,'nub');

			// 囤主题 www.tzhuti.com   检查积分情况
			if(!is_user_logged_in()){
				return '<div class="content-hide-tips pos-r t-c">
				<i class="iconfont zrz-icon-font-suo-copy pos-a"></i>
				...
				// 囤主题 www.tzhuti.com   检查用户积分是否足够，积分不足，提示充值
		...
		* 人民币支付可见
		*/
		// 囤主题 www.tzhuti.com   如果是金钱支付
		if($cap['key'] == 'rmb'){

			$need_rmb = $cap['val'];

			// 囤主题 www.tzhuti.com   检查用户余额
...
// 囤主题 www.tzhuti.com   当前文章是不是在幻灯中
