...
class zrz_media{
    ...
    public static $mime;// 囤主题 www.tzhuti.com   文件格式
    public static $type;// 囤主题 www.tzhuti.com   文件是什么类型？头像还是文章图片..
    public static $file;// 囤主题 www.tzhuti.com   文件
    public static $user_id;// 囤主题 www.tzhuti.com   当前用户ID
    public static $is_letter_avatar;// 囤主题 www.tzhuti.com   如果非空，则为用户名
    public static $subdir;// 囤主题 www.tzhuti.com   日期目录
    public static $cover_y;// 囤主题 www.tzhuti.com   用户封面图片裁剪的坐标
    public static $key;// 囤主题 www.tzhuti.com   文件名
    public static $keyi;// 囤主题 www.tzhuti.com   文件名，没有后缀
    ...
    public function __construct($file,$type = '',$user_id = 0,$is_letter_avatar = '',$cover_y = 0){
        ...
        // 囤主题 www.tzhuti.com   如果没有指定用户ID，则使用当前登陆的用户ID
        if(!$user_id){
        ...
        // 囤主题 www.tzhuti.com   是否是头像上传
        if($is_letter_avatar){
            self::$is_letter_avatar = $is_letter_avatar;
        }

        // 囤主题 www.tzhuti.com   获取文件的扩展名
        $path = self::$file['name'];
        self::$mime = pathinfo($path, PATHINFO_EXTENSION);

        // 囤主题 www.tzhuti.com   用户封面图片裁剪的Y坐标
        if($cover_y)
        self::$cover_y = $cover_y;

        // 囤主题 www.tzhuti.com   路径
        $path = wp_upload_dir();
        // 囤主题 www.tzhuti.com   $upload_dir = zrz_upload_dir($path);
        self::$subdir = trim($path['subdir'],'/');

        // 囤主题 www.tzhuti.com   文件名
        self::$key = self::get_media_key();
    }

    // 囤主题 www.tzhuti.com   获得媒体文件名
    public static function get_media_key(){

        if(self::$key) return self::$key;

        // 囤主题 www.tzhuti.com   如果是字母头像，文件名直接是用户名的 md5
        ...
        $ext = '.'.self::$mime;
        self::$keyi = $key;
        // 囤主题 www.tzhuti.com   如果是图片，为图片添加后缀，已示区分
        switch (self::$type) {
            case 'cover':
...
function zrz_upload_dir($param){
    ...
    if($yun_setting['host'] && $stream){
        if(isset($yun_setting['path']) && !empty($yun_setting['path'])){
            $param['basedir'] = $stream.':// 囤主题 www.tzhuti.com   '.trim($yun_setting['bucket'], '/').'/'.trim($yun_setting['path'],'/');
            ...
            $param['basedir'] = $stream.':// 囤主题 www.tzhuti.com   '.trim($yun_setting['bucket'], '/');
            $param['path'] = $param['basedir'] .'/'. trim($param['subdir'],'/');
