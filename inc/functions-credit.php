...
class Zrz_Credit_Message{
    ...
    private $type;// 囤主题 www.tzhuti.com   通知类型
    private $date;// 囤主题 www.tzhuti.com   时间
    private $hot;// 囤主题 www.tzhuti.com   是否增加活跃值
    ...
    private function credit_action($type = 0){
        ...
        // 囤主题 www.tzhuti.com   如果最后积分小于零，返回
        if($user_credit_new < 0){
            return false;
        }

        // 囤主题 www.tzhuti.com   写入积分,如果是积分抽奖和积分兑换则不受限制
    ...
    public function add_message($users = '',$credit = 0,$key = 0,$value = '',$type = 'add'){
        ...
        // 囤主题 www.tzhuti.com   如果设置了空，则不增加消息，保留活动报名通知
        if($credit === 'empty' && $this->type != 44) return false;

        // 囤主题 www.tzhuti.com   检查未读的历史数据
        ...
        // 囤主题 www.tzhuti.com   如果存在未读的历史数据
        if($check && $this->type != 13 && $this->type != 41 && $this->type != 30 && $this->type != 16 && $this->type != 45){
            ...
            // 囤主题 www.tzhuti.com   不存在未读的信息，直接添加
            $this->credit = 0;
            $this->credit_total = 0;
            $this->users = $users;
            $this->key = $key;
            ...
            // 囤主题 www.tzhuti.com   如果是申请提现，credit 字段则是提现的金额，不通过积分函数处理
