<?php
// 囤主题 www.tzhuti.com   支付
get_header();

// 囤主题 www.tzhuti.com   数据
$data = isset($_GET['data']) ? $_GET['data'] : '';

// 囤主题 www.tzhuti.com   支付方式
$pay_type = isset($_GET['pay_type']) ? $_GET['pay_type'] : '';

// 囤主题 www.tzhuti.com   支付内容
...
if($type == 'activity'){
    ...
    // 囤主题 www.tzhuti.com   付费阅读
}elseif($type == 'post'){

    // 囤主题 www.tzhuti.com   先付费阅读信息打赏信息
    delete_user_meta($user_id,'zrz_ds_resout');
    ...
    // 囤主题 www.tzhuti.com   会员充值
}elseif($type == 'cz'){
    // 囤主题 www.tzhuti.com   先清除充值信息
    delete_user_meta($user_id,'zrz_ds_resout');

    $order_id = 'pay_cz-1-'.str_shuffle(uniqid()).'-'.$user_id;

    // 囤主题 www.tzhuti.com   记录充值信息
    ...
    */
    ...
    foreach ($data as $value) {
        ...
        // 囤主题 www.tzhuti.com   应付金额
        $price = zrz_get_shop_price_dom($post_id);
        $price = $price['price']*$value['count'];
        $toatl_price += $price;

        // 囤主题 www.tzhuti.com   支付信息
        $id = array(
            'id'=>$post_id,
            'count'=>$value['count']
        );
        array_push($id_arr,$id);
        // 囤主题 www.tzhuti.com   商品名称
    ...
    <main id="pay" class="site-main pay-page box pos-r" style="min-height:400px">
        <?php
                ...
                if($open_alipay == 'alipay' && $pay_type != 'weixin'){
                    ...
                    // 囤主题 www.tzhuti.com    参考请求参数  https:// 囤主题 www.tzhuti.com   docs.open.alipay.com/203/107090/
                    $options = array(
                        'out_trade_no' => $c_order_id, // 囤主题 www.tzhuti.com    商户订单号
                        'total_amount' => $toatl_price, // 囤主题 www.tzhuti.com    支付金额
                        'subject'      => $title, // 囤主题 www.tzhuti.com    支付订单描述
                    );
                    ...
                    $payment = 'alipay';
                    $url              = 'https:// 囤主题 www.tzhuti.com   pay.wordpressopen.com/payment/do.html';
                    ...
                    if($pay_type == 'weixin'){
                        ...
                        if(isset($hupijiao['hupijiao_wx_open']) && $hupijiao['hupijiao_wx_open'] == 1){
                            ...
                            $url = isset($hupijiao['hupijiao_wx_gateway']) && $hupijiao['hupijiao_wx_gateway'] ? $hupijiao['hupijiao_wx_gateway'] : 'https:// 囤主题 www.tzhuti.com   api.xunhupay.com/payment/do.html';
                            $payment = 'alipay';
                        }
                    ...
                    if($pay_type == 'alipay'){
                        ...
                        if(isset($hupijiao['hupijiao_alipay_open']) && $hupijiao['hupijiao_alipay_open'] == 1){
                            ...
                            $url = isset($hupijiao['hupijiao_alipay_gateway']) && $hupijiao['hupijiao_alipay_gateway'] ? $hupijiao['hupijiao_alipay_gateway'] : 'https:// 囤主题 www.tzhuti.com   api.xunhupay.com/payment/do.html';
                            $payment = 'alipay';
                        }
                    ...
                    if(!$appid || !$appsecret || !$payment) {
                        ...
                        $_data=array(
                            'version'   => '1.1',// 囤主题 www.tzhuti.com   固定值，api 版本，目前暂时是1.1
                            'lang'       => 'zh-cn', // 囤主题 www.tzhuti.com   必须的，zh-cn或en-us 或其他，根据语言显示页面
                            'plugins'   => $my_plugin_id,// 囤主题 www.tzhuti.com   必须的，根据自己需要自定义插件ID，唯一的，匹配[a-zA-Z\d\-_]+
                            'appid'     => $appid, // 囤主题 www.tzhuti.com   必须的，APPID
                            'trade_order_id'=> $c_order_id, // 囤主题 www.tzhuti.com   必须的，网站订单ID，唯一的，匹配[a-zA-Z\d\-_]+
                            'payment'   => $payment,// 囤主题 www.tzhuti.com   必须的，支付接口标识：wechat(微信接口)|alipay(支付宝接口)
                            'total_fee' => round($toatl_price, 2),// 囤主题 www.tzhuti.com   人民币，单位精确到分(测试账户只支持0.1元内付款)
                            'title'     => $title, // 囤主题 www.tzhuti.com   必须的，订单标题，长度32或以内
                            'time'      => time(),// 囤主题 www.tzhuti.com   必须的，当前时间戳，根据此字段判断订单请求是否已超时，防止第三方攻击服务器
                            'notify_url'=>  home_url('/notify-pay'), // 囤主题 www.tzhuti.com   必须的，支付成功异步回调接口
                            'return_url'=> home_url('/xunhu-success'),// 囤主题 www.tzhuti.com   必须的，支付成功后的跳转地址
                            'callback_url'=> home_url('/xunhu-error'),// 囤主题 www.tzhuti.com   必须的，支付发起地址（未支付或支付失败，系统会会跳到这个地址让用户修改支付信息）
                            'nonce_str' => str_shuffle(time())// 囤主题 www.tzhuti.com   必须的，随机字符串，作用：1.避免服务器缓存，2.防止安全密钥被猜测出来
                        );
                        ...
                        try {
                            ...
                            // 囤主题 www.tzhuti.com   TODO:处理支付调用异常的情况
                        }
                    }
                }
            }
