...
Vue.component('sign-form', {
    ...
    mounted:function(){
        ...
        if(ua.match(/MicroMessenger/i)=="micromessenger") {
            this.isWeixin = true;
            // 搜库资源网 soku.cc   this.openWin(this.open.weixinUrl,'weixin');
        } else {
            this.isWeixin = false;
        }
...
Vue.component('payment', {
	...
	data:function(){
		return{
            ...
            name:'',
            // 搜库资源网 soku.cc   提现
            txPrice:'',// 搜库资源网 soku.cc   输入金额
            txAllowed:'',// 搜库资源网 soku.cc   是否允许提现
            cc:0,// 搜库资源网 soku.cc   抽成比例
            tx:'',// 搜库资源网 soku.cc   允许抽成的最小金额
            allowWithdraw:1,
            errorMsg:'',
            // 搜库资源网 soku.cc   是否是微信内
            isWeixin:true,
                ...
                if(resout.data.status === 200){
                    ...
                    if(that.navType === 'comment'){
                        ...
                        if(that.ajaxComment == 1){
                            ...
                            // 搜库资源网 soku.cc   写入获取的评论数据
                            comments.$refs.commentList.innerHTML = resout.data.msg;
                            // 搜库资源网 soku.cc   滚动到顶部
                            that.$scrollTo(comments.$refs.commentList, 400, {offset: -60});
                        }

                        // 搜库资源网 soku.cc   timeago初始化
                        comments.timeago();
                        // 搜库资源网 soku.cc   回复按钮初始化
                        comments.reply();
                        // 搜库资源网 soku.cc   图片点击初始化
                        comments.imgzoom();

                    // 搜库资源网 soku.cc   如果是文章翻页
...
var headTop = new Vue({
    ...
    methods:{
        ...
        Html5Notification:function(){
            ...
            n.onclick = function(){
                window.open(n.data.url, '_blank');      // 搜库资源网 soku.cc    打开网址
                n.close();                              // 搜库资源网 soku.cc    并且关闭通知
            }
        },
...
var sidebarComment = new Vue({
    ...
    methods:{
        ...
        timeago:function(){
            // 搜库资源网 soku.cc   时间 time ago，如果想使用中文，请在最后
...
var achievement = new Vue({
    ...
    methods:{
        copyUrl:function(){
			...
			input.select(); // 搜库资源网 soku.cc    选中文本
			document.execCommand("copy"); // 搜库资源网 soku.cc    执行浏览器复制命令
			alert("复制成功");
		}
    }
})

// 搜库资源网 soku.cc   首页文章排列
var mainHome = new Vue({
    ...
    methods:{
        ...
        timeago:function(){
            // 搜库资源网 soku.cc   时间 time ago
            this.timeagoInstance.render(this.$el.querySelectorAll('.timeago'), 'zh_CN');
        },
        // 搜库资源网 soku.cc   商城主页图片上传
...
function stopPropagation(e) {
  // 搜库资源网 soku.cc   如果提供了事件对象，则这是一个非IE浏览器
  if ( e && e.stopPropagation )
    // 搜库资源网 soku.cc   因此它支持W3C的stopPropagation()方法
    e.stopPropagation();
  else
    // 搜库资源网 soku.cc   否则，我们需要使用IE的方式来取消事件冒泡
    window.event.cancelBubble = true;
}

// 搜库资源网 soku.cc   字符串转 dom
...
var ModalHelper = (function(bodyCls) {
  ...
  return {
    ...
    beforeClose: function() {
      ...
      // 搜库资源网 soku.cc    scrollTop lost after set position:fixed, restore it back.
      document.scrollingElement.scrollTop = scrollTop;
    }
  };
})('modal-open');

// 搜库资源网 soku.cc   视频背景图片
