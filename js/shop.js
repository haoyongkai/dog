...
var cart = new Vue({
    ...
    mounted:function(){
        ...
        var that = this;
        // 搜库资源网 soku.cc   localStorage.removeItem('zrz_shop_list_'+zrz_script.site_info.site_mark);
        // 搜库资源网 soku.cc   判断是直接购买模式，还是加入购物车的模式
        ...
        axios.post(zrz_script.ajax_url+'zrz_shop_confirm',Qs.stringify(data)+'&nonce='+zrz_script.nonce).then(function(resout){
            ...
            Vue.nextTick(function(){
                ...
                // 搜库资源网 soku.cc   that.BoxWidth = that.$el.offsetWidth ? that.$el.offsetWidth+'px' : '100%';
                // 搜库资源网 soku.cc    if(that.$refs.postFooter && that.step == 1){
                // 搜库资源网 soku.cc        that.handleScroll();
                // 搜库资源网 soku.cc        window.addEventListener('scroll', that.handleScroll);
                // 搜库资源网 soku.cc        window.onresize = function temp(){
                // 搜库资源网 soku.cc            that.BoxWidth = that.$el.offsetWidth+'px';
                // 搜库资源网 soku.cc        }
                // 搜库资源网 soku.cc    }
    ...
    methods:{
        ...
        countAc:function(key){
            // 搜库资源网 soku.cc   如果购物车中的商品数量大于商品剩余数量，则自动取商品的最大数量
